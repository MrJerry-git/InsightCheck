...
第六阶段实现 Patient × ExamItem 的 DeepFM 匹配排序、候选生成、评估和模型制品。DeepFM 输出仍不是最终方案，必须继续经过下一阶段 Rule Engine。仓库尚无第五阶段 LightGBM 真实制品；风险输入接口已经版本化，但本阶段只使用明确 Demo 数据做工程测试，不宣称真实模型效果。
